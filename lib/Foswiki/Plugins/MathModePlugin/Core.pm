...
sub new {
    ...
    my $this = {
       ...
       # huge or Huge
        ...
        imagePrefix => $Foswiki::cfg{MathModePlugin}{ImagePrefix}
          || '_MathModePlugin_',
        ...
        latexFontSize => $Foswiki::cfg{MathModePlugin}{LatexFontSize}
          || 'normalsize',
...
sub init {
    ...
    $this->{imageFileNameLength} =
      $this->{hashCodeLength} +
      length( $this->{imageType} ) +
      length( $this->{imagePrefix} ) + 1;
...
sub handleMath {
    ...
    my $hashCode =
      md5_hex( $text . $this->{fgColors}{$text} . $this->{bgColor} . $size );
    ...
    my $result =
      ...
      . $args . ' />';
    $result =
        "<$container class='mmpContainer' align='center'>" 
      . $result
      . "<\/$container>"
      unless $inlineFlag == 2;
...
sub checkImages {
    ...
    foreach my $fileName (@files) {
        ...
        next
          unless $hashCode =~ /^$this->{imagePrefix}(.*)\.$this->{imageType}$/;
        $hashCode = $1;
...
PREAMBLE
    ...
    while ( my ( $key, $value ) = each( %{ $this->{hashedMathStrings} } ) ) {
        $imageNumber++;
        print $tempFile "{\n";
        print $tempFile "\\color"
          . formatColorSpec( $this->{fgColors}{$value} ) . "\n"
        ...
        print $tempFile
          "\\begin{$environment}\\displaystyle $value\\end{$environment}\n";
        print $tempFile "}\n";
    ...
    if ( $this->{latex2Img} ) {
        ...
        ( $data, $exit ) =
          Foswiki::Sandbox->sysCommand( $latex2ImgCmd,
            FILENAME => "$tempFile" );
    }
    else {
        $exit = 1;
        $data =
"MathModePlugin cannot find the latex2img script. Please check the settings in configure";
    }
    ...
    if ($exit) {
        $msg =
            '<div class="foswikiAlert">Error during latex2img:<pre>' 
          . $data
          . '</pre></div>';
    }
    else {
        ...
        while ( ( my $key, my $value ) = each(%imageFile) ) {
            ...
            move( $source, $target )
              ;    # or die "can't move $source to $target: $@";
        }
    }
